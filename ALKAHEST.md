...
The overlap deserves to be named for what it is, because it is the document's own thesis handed over by history. Maria the Jewess wrote in Hellenistic Alexandria in the first centuries of the common era, and Zosimos — the figure whose Portico opens this piece — is one of the principal sources through whom her axiom survives at all; she is inside the exact lineage the argument here is built on. Laozi wrote in Warring States China, four to seven centuries earlier, with no channel to Alexandria and none from it to him. Two traditions with no contact between them produced the identical four-term unfolding of distinction, and neither had the algebra to write it as an equation. That is not a borrowing to be traced and it is not a coincidence to be explained away. It is the same signature section 11 makes explicit: a structural law reappearing unbidden in an instrument built for an entirely different purpose, which is the clearest confirmation a structural claim can get. The convergence of the two texts is, first, a fact about the history of ideas, legible by inspection: one becomes two, two becomes three, and the third yields the fourth, in both mouths. The Grammar's contribution is on the Tao side, where it can be checked — `tao_cosmogony` imscribes to $O_\infty$, and the catalog entry records the identity in its own description, naming Maria's axiom as the structure it shares. (A caution the document's own standard demands: the entry catalogued under Maria's name, `mary_prophetess`, is not the axiom but the later *Practise of Mary the Prophetess*, a practical dialogue that reads at $O_0$; the axiom itself is not separately catalogued, so the four-term identity here rests on reading the two texts, not on a second machine run.) The shared structure was always the reason the words rhymed.
