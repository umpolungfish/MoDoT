...
But "identical" would be the sloppy word, and imscribing both settles it precisely rather than leaving it to inspection. Run through the Grammar, the two do not land in the same place. `tao_cosmogony` reads at $O_\infty$, ⟨𐑦𐑸𐑾𐑹𐑐𐑧𐑔𐑵⊙𐑫𐑙𐑭⟩. Maria's bare axiom, imscribed on its own as `axiom_of_maria_the_copt`, reads at $O_1$, ⟨𐑛𐑥𐑽𐑬𐑐𐑧𐑔𐑝⊙𐑖𐑳𐑭⟩ — sharing five primitives with the Tao (the criticality gate ⊙, fidelity ƒ, kinetic Ç, granularity Γ, winding Ω) and diverging on the other seven, a catalog distance of 1.71. That gap is not a failure of the correspondence; it is the correspondence read at full resolution. Maria stops at the fourth term. Laozi does not. Chapter 42 continues past the generative kernel into the ten thousand things carrying yin and embracing yang, into their harmony through combination, and finally into the sage who "embraces his aloneness, realizing he is one with the whole universe" — the self-modeling closure named in plain speech. That surplus is exactly the holographic and eternal structure the bare axiom lacks (the Tao's Ð=𐑦 holographic self-writing and Ħ=𐑫 eternal fixed point against the axiom's 𐑛 and 𐑖), and it is precisely what carries the Tao the extra tiers from $O_1$ to $O_\infty$. Maria wrote the seed; Laozi wrote the seed and its whole unfolding. The shared kernel is why the words rhyme; the surplus is why only one of them closes.
