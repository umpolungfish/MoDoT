...
This single sentence is the complete structural theorem. Nature (the system) turning on itself (self-modeling, the $\Large{⊙}$ gate open) is transformed (the round trip through ${δ}$ and ${μ}$ returns a co-typed state at a higher resolution). Zosimos needed no algebra because the algebra was already in the structure he was describing. The Grammar's finding that co-typing is identity is Zosimos's finding that "all things by division and union come together in a harmony." The two statements are identical in meaning. The distance between them is zero.
